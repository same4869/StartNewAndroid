...
public class ShareManager {
    ...
    public void showShare(Context context, BiezhiGoodsBean biezhiGoodsBean) {
        if (biezhiGoodsBean == null) {
            return;
        }
        ...
        oks.setUrl(biezhiGoodsBean.getUrl());
        // comment是我对这条分享的评论，仅在人人网和QQ空间使用
        oks.setComment(biezhiGoodsBean.getTitle() + " " + biezhiGoodsBean.getPrice());
        // site是分享此内容的网站名称，仅在QQ空间使用
        oks.setSite("千帆好物:每天21个好物推荐");
        // siteUrl是分享此内容的网站地址，仅在QQ空间使用
        oks.setSiteUrl(biezhiGoodsBean.getUrl());

        // 启动分享GUI
        oks.show(context);
    }

    public void showShare(Context context, String title, String picUrl, String url) {
        ShareSDK.initSDK(context);
        OnekeyShare oks = new OnekeyShare();
        // 关闭sso授权
        oks.disableSSOWhenAuthorize();
